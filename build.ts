...
(async () => {
	try {
		...
		try {
			...
			console.error(
				`${colors.red}Error building UMD module:${colors.reset}`,
		...
		try {
			...
			console.error(
				`${colors.red}Error building ES module:${colors.reset}`,
				(err as Error).message,
			);
			...
			try {
				...
				await fs.promises.writeFile(
					path.resolve(__dirname, "lib", "rplayer.es.js"),
					`// Transpiled version of src/lib/index.ts\n${indexContent.replace(/\.ts/g, ".js")}`,
					"utf8",
				);
				console.log(`${colors.green}✓ Manual fallback ES file created${colors.reset}`);
			} catch (readWriteError) {
				console.error(
					`${colors.red}Error creating manual ES file:${colors.reset}`,
					(readWriteError as Error).message,
				);
			}
		...
		try {
			if (fs.existsSync(path.resolve(__dirname, "src", "lib", "rplayer.d.ts"))) {
				...
				console.warn(
					`${colors.yellow}TypeScript definition file not found in src/lib/${colors.reset}`,
				);
			}
		} catch (err) {
			console.error(
				`${colors.red}Error copying TypeScript definition file:${colors.reset}`,
				(err as Error).message,
			);
		}

		// 4. Check generated files
		const requiredFiles = ["lib/rplayer.es.js", "lib/rplayer.umd.min.js", "lib/rplayer.d.ts"];
		...
		try {
			...
			console.error(
				`${colors.red}Error reading lib directory:${colors.reset}`,
				(err as Error).message,
			);
		}
		...
		if (missingFiles.length > 0) {
			...
			for (const file of missingFiles) {
				...
				if (file === "lib/rplayer.es.js") {
					try {
						...
						console.error(
							`${colors.red}Failed to rebuild ${file}:${colors.reset}`,
							(err as Error).message,
						);
					}
					...
					try {
						...
						console.error(
							`${colors.red}Failed to recreate ${file}:${colors.reset}`,
							(err as Error).message,
						);
					}
				}
			...
			if (stillMissing.length > 0) {
				console.warn(
					`${colors.yellow}Warning: Some files are still missing but build will continue:${colors.reset}`,
				);
		...
		console.error(`${colors.red}Build error:${colors.reset}`, (error as Error).message);
		process.exit(1);
	}
})();
